...
// Safely escape data for TeX parsing
function escapeTeX(data) {
...
// Iterate through given TeX data and tag notable words
function emphasizeTeX(data, metadata) {
    var tempData = data
    metadata.keywords.forEach(function(keyword, index) {
        tempData = tempData.replace(
            new RegExp(escapeRegExp(escapeTeX(keyword)), 'g'),
            '{\\bf ' + escapeTeX(keyword) + '}'
        )
    })
    return tempData
}
...
function generateTeX(data) {
    var renderedTeX = swig.renderFile(path.join(templateDir, 'tex', 'resume.tex'), trimJSON(data))
    return emphasizeTeX(renderedTeX, data.metadata)
}
