...
// Initialize AWS module for a given region
function initAWS(region, table) {
...
function routerAWS(req, res) {
    if (!aws.region) {
        response.send(400, 'AWS response router cannot determine AWS region')
        return
    ...
    aws.getResumeData(module.exports.table, function(err, data) {
        if (err) {
        ...
        serveResume(req, res, resumeData)
    })
}
...
function routerLocal(req, res) {
    ...
    fs.readFile(module.exports.resume, function(err, data) {
        if (err) {
            console.error(err, err.stack)
